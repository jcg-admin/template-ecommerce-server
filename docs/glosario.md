...
| Termino | Significado |
|---------|-------------|
| **ACME** | Automatic Certificate Management Environment. Protocolo IETF (RFC 8555) para gestion automatica de certificados X.509. Let's Encrypt lo usa. |
| **acme.sh** | Cliente ACME en bash puro. Alternativa a certbot. Usado en este repo por simplicidad y nulas dependencias Python. |
| **API_UPSTREAM** | Variable de entorno definida en `.env`. Apunta al backend al que Nginx reverse-proxy-eara `/api/*`. Vacio por defecto: el server NO asume tecnologia backend. |
| **arc42** | Estandar de documentacion de arquitectura. Junto con PROC-GESTION-001 forma la base de la documentacion de este repo. |
...
| **dist/** | Output del build de webpack en `template-e-comerce-ui`. Producido por `npm run build`. Es lo que Nginx sirve directamente desde filesystem. |
| **fail2ban** | Demonio que monitoriza logs y banea IPs con patrones de abuso. En este repo: jails sshd + nginx-limit-req + nginx-botsearch. |
| **HSTS** | HTTP Strict Transport Security. Header HTTP que instruye al browser a usar SOLO HTTPS por un periodo de tiempo. En este repo: 1 año + includeSubDomains + preload. |
...
| **mod_wsgi** | Modulo Apache que embebe un interprete Python. Usado por el referente para servir Django. **NO usado en este repo**: la justificacion completa de descartarlo esta en `docs/arquitectura.md`. |
| **Nginx** | Servidor web y reverse proxy. **Elegido** sobre Apache para este repo. Justificacion en `docs/arquitectura.md`. |
...
| **rsync** | Herramienta de sincronizacion incremental de archivos. Usada para desplegar el `dist/` del UI al server. |
...
| **template-e-comerce-ui** | Repositorio del UI React, **con guion** entre `e` y `comerce`. Naming historico. Repo de https://github.com/jcg-admin/template-e-comerce-ui. |
...
| **WSGI** | Web Server Gateway Interface. Estandar Python (PEP 3333) para comunicar web servers con apps Python. Usado por el referente con Django; **NO usado en este repo**. |
...
| **Yorubá** | Identificador de marca aparecido en el referente (`PracticaYoruba`). Sin relacion con este repo; solo nota historica. |

## Comparacion rapida de cuentas Linux

```
deploy:        UID 1000, sudo,     login,    posee nada
infra:         UID 1001, granular, login,    posee nada
develop:       UID 1002, NO sudo,  login,    posee Clase A (codigo)
svc-backups:   UID  999, NO sudo,  nologin,  posee Clase B (backups)
www-data:      UID variable, NO sudo, nologin, ejecuta Nginx workers
```
...
Clase A: /srv/repos/ecom/template-e-comerce-ui
         ...
         perms: 755/644 (world-readable)
         contiene: codigo del UI

Clase B: /srv/backups/project
         owner: svc-backups:svc-backups
         perms: 755
         contiene: backups del proyecto
```
