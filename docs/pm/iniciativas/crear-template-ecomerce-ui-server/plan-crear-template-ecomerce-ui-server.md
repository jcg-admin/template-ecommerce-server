...
con la propuesta del analisis previo (`template-e-comerce-ui/
docs/desarrollo/analisis-servidor-para-template.md`). Cada fase
...
| **F0a** | Validaciones iniciales | 30 min | F0 cerrada | Decision Nginx ratificada, repo de referencia confirmado |
...
| **F10** | Documentacion | 60 min | F9 cerrada | `docs/operaciones.md`, `docs/upgrade-server-systemless.md` |
| **F11** | Integracion con template-e-comerce-ui | 30 min | F10 cerrada | Commit en el repo UI documentando la relacion |
...
- Repo de referencia `jcg-admin/e-comerce-server` clonado en
  `/tmp/references/e-comerce-server/` (sirve de referencia
...
1. El repo `template-ecomerce-ui-server` queda listo para clone
   y ejecucion en un Ubuntu 24.04 real.
