...
| T-302 | Crear `config/nginx/template-https.conf` (SSL + UI static + SPA catch-all + reverse proxy + headers) | 40 min | **Cerrada** | Archivo |
...
| T-402 | `provisioners/nginx/setup_vhost.sh`: reemplazar `%%VAR%%` en configs, validar nginx -t, recargar | 60 min | **Cerrada** | Script |
| T-403 | Tests basicos manuales locales (en WSL2 o contenedor) | 20 min | **Cerrada** | Verificacion |
...
| T-602 | `provisioners/security/setup_ssh_hardening.sh`: portacion 1:1 con cambio de marca | 40 min | **Cerrada** | Script |
